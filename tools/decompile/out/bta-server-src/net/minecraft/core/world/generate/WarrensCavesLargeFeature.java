...
public class WarrensCavesLargeFeature extends LargeFeature {
   ...
   protected void generateCave(
      ...
      if (endPos <= 0) {
         int maxLength = this.radiusChunk * 24 - 16;
         endPos = maxLength - random.nextInt(maxLength / 8);
      }

      boolean noRotate = random.nextInt(32) == 0;
      if (noRotate) {
         yRot = yRot / (float) (Math.PI * 2) * 4.0F;
         yRot = Math.round(yRot);
         yRot = yRot * (float) (Math.PI * 2) / 4.0F;
      }

      int branchPos = random.nextInt(endPos / 2) + endPos / 4;
      boolean sharpRotVer = random.nextInt(6) == 0;
      ...
      while (startPos < endPos) {
         ...
         if (!noRotate) {
            ...
            int addRandom = random.nextInt(4);
            if (addRandom == 0) {
               yRot += random.nextFloat() * (float) Math.PI - (float) (Math.PI / 2);
            }
         }
   ...
   protected void doGeneration(World world, int chunkX, int chunkZ, int baseChunkX, int baseChunkZ, ChunkGeneratorResult result) {
      int cavesToGenerate = rand.nextInt(rand.nextInt(rand.nextInt(100) + 1) + 1);
      ...
      for (int i = 0; i < cavesToGenerate; i++) {
         ...
         for (int l1 = 0; l1 < numBranches; l1++) {
            ...
            float initialRadius = rand.nextFloat() * 1.5F + rand.nextFloat();
