...
public class WorldFeaturePumice extends WorldFeature {
   ...
   public WorldFeaturePumice(int numberOfBlocks) {
      this(numberOfBlocks, Blocks.COBBLE_NETHERRACK);
   }

   @MethodParametersAnnotation(names = {"numberOfBlocks", "block"})
   public WorldFeaturePumice(int numberOfBlocks, Block<?> block) {
   ...
   public boolean place(World world, Random random, int x, int y, int z) {
      if (world.getBlockMaterial(x, y, z) == Material.lava && world.getBlockId(x, y + 1, z) == 0) {
         ...
         double d4 = y - random.nextInt(2) + 1;
         double d5 = y - random.nextInt(2) + 1;
                     ...
                     if (!(d12 * d12 + d13 * d13 >= 1.0)) {
                        for (int i3 = k1; i3 <= j2; i3++) {
                           ...
                           if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0 && world.getBlockId(k2, l2, i3) == this.generateInId) {
                              world.setBlock(k2, l2, i3, Blocks.PUMICE_WET.id());
                           }
                        }
                     }
                  }
