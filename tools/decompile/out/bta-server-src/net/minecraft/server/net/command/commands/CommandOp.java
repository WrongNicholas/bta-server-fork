...
public class CommandOp implements CommandManager.CommandRegistry {
   private static final SimpleCommandExceptionType FAILURE = new SimpleCommandExceptionType(
      () -> I18n.getInstance().translateKey("command.commands.op.exception_failure")
   );

   @Override
   public void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register(
         ArgumentBuilderLiteral.<CommandSource>literal("op")
                  ...
                  .executes(c -> {
                     ...
                     for (Entity entity : entities) {
                        ...
                        if (!player.isOperator()) {
                           oppedSomeone = true;
                           server.playerList.opPlayer(player.uuid);
                           source.sendTranslatableMessage("command.commands.op.success", player.username);
                           source.sendTranslatableMessage(player, "command.commands.op.success_receiver");
                        }
