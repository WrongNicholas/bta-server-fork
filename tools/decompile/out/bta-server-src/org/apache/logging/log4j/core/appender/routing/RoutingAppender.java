...
public final class RoutingAppender extends AbstractAppender {
   ...
   private final ConcurrentMap<String, RoutingAppender.CreatedRouteAppenderControl> createdAppenders = new ConcurrentHashMap<>();
   ...
   private final ConcurrentMap<String, RoutingAppender.RouteAppenderControl> referencedAppenders = new ConcurrentHashMap<>();
      ...
      for (Route route : this.routes.getRoutes()) {
         if (route.getAppenderRef() != null) {
            ...
            if (appender != null) {
               ...
               this.referencedAppenders.put(key, new RoutingAppender.ReferencedRouteAppenderControl(appender));
            } else {
   ...
   public boolean stop(final long timeout, final TimeUnit timeUnit) {
      ...
      for (Entry<String, RoutingAppender.CreatedRouteAppenderControl> entry : this.createdAppenders.entrySet()) {
         Appender appender = entry.getValue().getAppender();
   ...
   public void append(LogEvent event) {
      ...
      String key = pattern != null
         ? this.configuration.getStrSubstitutor().replace(event, pattern)
         : (this.defaultRoute.getKey() != null ? this.defaultRoute.getKey() : "ROUTING_APPENDER_DEFAULT");
      RoutingAppender.RouteAppenderControl control = this.getControl(key, event);
      if (control != null) {
         try {
   ...
   private synchronized RoutingAppender.RouteAppenderControl getControl(final String key, final LogEvent event) {
      RoutingAppender.RouteAppenderControl control = this.getAppender(key);
      if (control != null) {
         control.checkout();
         return control;
      } else {
         ...
         if (route != null) {
            ...
            RoutingAppender.CreatedRouteAppenderControl created = new RoutingAppender.CreatedRouteAppenderControl(app);
            control = created;
   ...
   private RoutingAppender.RouteAppenderControl getAppender(final String key) {
      RoutingAppender.RouteAppenderControl result = this.referencedAppenders.get(key);
      return result == null ? this.createdAppenders.get(key) : result;
   }
   ...
   public void deleteAppender(final String key) {
      ...
      RoutingAppender.CreatedRouteAppenderControl control = this.createdAppenders.remove(key);
      if (null != control) {
   ...
   private static final class CreatedRouteAppenderControl extends RoutingAppender.RouteAppenderControl {
   ...
   private static final class ReferencedRouteAppenderControl extends RoutingAppender.RouteAppenderControl {
