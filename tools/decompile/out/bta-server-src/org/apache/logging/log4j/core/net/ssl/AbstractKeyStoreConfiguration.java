...
public class AbstractKeyStoreConfiguration extends StoreConfiguration<KeyStore> {
   ...
   protected KeyStore load() throws StoreConfigurationException {
      ...
      } catch (CertificateException var34) {
         LOGGER.error("No Provider supports a KeyStoreSpi implementation for the specified type {} for location {}", this.keyStoreType, loadLocation, var34);
         throw new StoreConfigurationException(loadLocation, var34);
      } catch (NoSuchAlgorithmException var35) {
         LOGGER.error("The algorithm used to check the integrity of the keystore cannot be found for location {}", loadLocation, var35);
         throw new StoreConfigurationException(loadLocation, var35);
      } catch (KeyStoreException var36) {
         LOGGER.error("KeyStoreException for location {}", loadLocation, var36);
         throw new StoreConfigurationException(loadLocation, var36);
      } catch (FileNotFoundException var37) {
         LOGGER.error("The keystore file {} is not found", loadLocation, var37);
         throw new StoreConfigurationException(loadLocation, var37);
      } catch (IOException var38) {
         LOGGER.error("Something is wrong with the format of the keystore or the given password for location {}", loadLocation, var38);
         throw new StoreConfigurationException(loadLocation, var38);
      } finally {
         if (password != null) {
